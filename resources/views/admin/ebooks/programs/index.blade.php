...
@section('title', 'E-Book Programs')
@section('page-title', 'E-Book Programs')

@section('content')
<div class="container-fluid ebook-programs-page">
    ...
    <section class="programs-hero">
        <div class="hero-copy">
            ...
            <div>
                <span class="hero-eyebrow">E-Book Management</span>
                <h2>Academic Programs</h2>
                <p>Organize the programs displayed in your E-Book Collection.</p>
            </div>
        </div>

        <a href="{{ route('admin.ebook-programs.create') }}" class="btn-add-program">
    ...
    @if(session('success'))
        <div class="alert program-alert alert-dismissible fade show" role="alert">
            <span class="alert-icon"><i class="bi bi-check-lg"></i></span>
            <div>
    ...
    <section class="programs-panel">
        <div class="panel-toolbar">
            <div>
                ...
                <p>
                    {{ $programs->total() }}
                    {{ \Illuminate\Support\Str::plural('program', $programs->total()) }} found
                </p>
            </div>

            <form method="GET" action="{{ route('admin.ebook-programs.index') }}" class="program-search">
            ...
            <table class="table programs-table align-middle mb-0">
                ...
                <tbody>
                    @forelse($programs as $program)
                        <tr>
                            <td class="row-number">
                                {{ ($programs->firstItem() ?? 1) + $loop->index }}
                            </td>
                            ...
                            <td class="text-center">
                                <span class="status-badge {{ (int) $program->status === 1 ? 'active' : 'hidden' }}">
                                    <span></span>
                                    {{ (int) $program->status === 1 ? 'Active' : 'Hidden' }}
                                </span>
                            </td>

                            <td>
                                <div class="table-actions">
                                    <a href="{{ route('admin.ebook-programs.edit', $program) }}"
                                    ...
                                    <form
                                        action="{{ route('admin.ebook-programs.destroy', $program) }}"
                                        method="POST"
                                        onsubmit="return confirm('Delete &quot;{{ addslashes($program->title) }}&quot;? This action cannot be undone.');">
                                        @csrf
                                        @method('DELETE')
        ...
        @if($programs->hasPages())
            <div class="panel-footer">
                <p>
                    Showing {{ $programs->firstItem() }}–{{ $programs->lastItem() }}
                    of {{ $programs->total() }}
                </p>
